...
UPDATE RAW_PTY WITH (UPDLOCK) 
SET NAME='ORDER OF SHIPPER' 

select * from archive_raw_pty (nolock)
WHERE LTRIM(RTRIM(ISNULL(NAME,'')))='' 
AND LTRIM(RTRIM(ISNULL(ADDR_1,'')))=''
...
UPDATE A WITH (UPDLOCK) SET A.NAME=B.NAME,A.ADDR_1=B.ADDR_1,A.ADDR_2=B.ADDR_2,A.ADDR_3=B.ADDR_3,A.ADDR_4=B.ADDR_4
...
select * 
FROM archive_RAW_PTY A JOIN archive_RAW_PTY B   WITH (NOLOCK)   ON A.BOL_ID=B.BOL_ID
WHERE A.SOURCE='N' AND B.SOURCE='C' AND REPLACE(A.NAME,' ','') LIKE '%ASCONSIGNEE%'
